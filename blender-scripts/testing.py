...
def open_csv(os='win', csv_file=filename):
    ...
    if os == 'win' or os == 'windows':
        path = path_win+csv_file
    elif os == 'lin' or os == 'linux':
        path = path_lin+csv_file
    else: 
        print("Wrong operating system argument provided. \nPlease choose between 'windows' or 'linux'.")
        
    file = open(path)
    inFile = csv.reader(file, delimiter=',', quotechar='"')
    
    # Normalized (0, 1) (x, y, z)-coordinates from csv
    vertices = [(float(r[0]), float(r[1]), float(r[2])) for r in inFile]
    # Edges as specified in mediapipe facemesh sourcecode
...
def center_to_origin(data, method='object'):
    ''' Move the center of the data mesh to the origin by recalculating the vertices. 
    
        Method : 'object' - use the center of the vertices to determine the origin.
        Method : 'global' - use the normalized [0, 1] field to determine the origin.
    '''
    ...
    if method == 'object':
        ...
        mean = np.array([0.5, 0.5, 0])
    else:
        print("Wrong method argument provided. \nPlease choose between 'object' or 'global'.")
...
ob = bpy.data.objects.new("test_obj", mesh)
